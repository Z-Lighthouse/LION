    ...
    methods
        ...
        function compare_single(obj, ground_exec, next_exec, next_exec_idx)
            ...
            for i = 1 : numel(blocks)
                ...
                % 再来个accData
                if covcfg.SIMACC_IF
                    num_data_3 = numel(data_2.accData);
                    num_time_3 = numel(data_2.accTime);
                    % 比较一下
                    if obj.compare_len && ...
                            (num_data_3 ~= num_data_2 || num_time_3 ~= num_time_2)
                        obj.l.error('accTime/data len mismatch');
                        e =  MException('RandGen:SL:CompareErrorLen',...
                            sprintf('Len mismatch %s: data3: %d; data2: %d; time3: %d; time2:%d',...
                            bl_name, num_data_3, num_data_2, num_time_3, num_time_2)) ;

                        obj.handle_comp_err(obj.r.comp_diffs, bl_name,...
                            next_exec, [], [], e, next_exec_idx);
                    else
                        t_3 = data_2.accTime(numel(data_2.accTime));
                        t_2 = data_2.Time(numel(data_2.Time));
                        try
                            d_3 = data_2.accData(numel(data_2.accData));
                            d_2 = data_2.Data(numel(data_2.Data));
                        catch e
                            disp(e)
                            rethrow(e);
                        end
                        if (isnan(d_3) && isnan(d_2)) || (d_3 == d_2)
                            %                         fprintf('Data No Mismatch\n');
                        else
                            obj.l.error('accData Mismatch!');
                            e = MException('RandGen:SL:CompareErrorData',...
                                sprintf('accData mismatch %s: data3: %f; data2: %f; len3: %d; len2:%d',...
                                bl_name, d_3, d_2, num_data_3, num_data_2 )) ;
                            obj.handle_comp_err(obj.r.comp_diffs, bl_name,...
                                next_exec, d_3, d_2, e, next_exec_idx);
                        end
                        if t_3 == t_2
                            %                         fprintf('Time No Mismatch\n');
                        else
                            obj.l.error('accTime Mismatch!');
                            e = MException('RandGen:SL:CompareErrorTime',...
                                sprintf('accTime mismatch %s: time3: %f; time2: %f; len3: %d; len2:%d',...
                                bl_name, t_3, t_2, num_time_3, num_time_2 )) ;

                            obj.handle_comp_err(obj.r.comp_diffs, bl_name,...
                                next_exec, t_3, t_2, e, next_exec_idx);
                        end
                    end
                end
